...
- `POST /api/behavior-metrics/` — публичный приём поведенческих метрик с главной страницы;
- `GET /api/behavior-metrics/stats` — статистика визитов и точки курсора для хитмэпа, только с JWT.
...
### Статистика поведения посетителей

Главная страница раз в секунду отправляет накопленные метрики визита в `POST /api/behavior-metrics/`: секунды на странице, счётчик кликов по кнопкам (JSON вида `{"Рассчитать обслуживание": 2}`) и позицию курсора (JSON точек в процентах окна). Один визит — одна строка в таблице `behavior_metrics`: повторные запросы того же `session_id` обновляют существующую запись, а не создают новую.

Кнопка «Статистика» в админ-панели открывает полноэкранное окно со средним и максимальным временем на странице за день, неделю и месяц, а также canvas-хитмэпом позиций курсора: чем чаще посетители держат курсор в области, тем «теплее» она подсвечена.
